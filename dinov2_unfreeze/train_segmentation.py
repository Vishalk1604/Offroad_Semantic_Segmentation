...
NUM_WORKERS            = 4
PIN_MEMORY             = True
BACKBONE_UNFREEZE_EPOCH = 10   # Unfreeze last DINOv2 block at this epoch (1-indexed)
BACKBONE_LR            = LR * 0.05  # Much lower LR for pretrained weights (3e-4 * 0.05 = 1.5e-5)
...
def unfreeze_last_dino_block(backbone, optimizer, backbone_lr):
    """
    Unfreeze the last transformer block of DINOv2 and inject its parameters
    into the optimizer as a new param group with a conservative LR.
    Returns the number of newly trainable parameters.
    """
    # DINOv2 stores transformer blocks in backbone.blocks (nn.ModuleList)
    last_block = backbone.blocks[-1]

    for p in last_block.parameters():
        p.requires_grad_(True)

    backbone.train()  # allow BN/dropout in the unfrozen block to behave correctly
    # Keep all other backbone blocks frozen + eval
    backbone.blocks[:-1].eval()
    for blk in backbone.blocks[:-1]:
        for p in blk.parameters():
            p.requires_grad_(False)

    # Also keep patch embed, norm, etc. frozen
    ...
    # We don't modify the optimizer here; optimizer should have been created
    # with the backbone last block as a separate param-group already. Just
    # enable gradients for the block and return its size.
    n_new = sum(p.numel() for p in last_block.parameters())
    return n_new
...
def main():
    ...
    # Prepare optimizer with two param-groups: head and (initially frozen)
    # last DINOv2 block. Including the block here avoids adding param groups
    # at runtime which would break OneCycleLR's internal state.
    backbone_last_params = list(backbone.blocks[-1].parameters())
    optimizer = optim.AdamW([
        {'params': head.parameters(), 'lr': LR},
        {'params': backbone_last_params, 'lr': BACKBONE_LR},
    ], weight_decay=1e-4)

    steps_per_epoch = max(1, len(train_loader) // ACCUM_STEPS)
    # Supply per-group max_lr so scheduler knows rates for both groups
    ...
    backbone_unfrozen = False   # tracks whether last block has been unlocked

    print(f"\nStarting {N_EPOCHS} epochs | EffBatch={BATCH_SIZE*ACCUM_STEPS} | AMP=ON")
    print(f"Last DINOv2 block will unfreeze at epoch {BACKBONE_UNFREEZE_EPOCH} "
          f"(backbone_lr={BACKBONE_LR:.2e})")
    print("=" * 70)

    for epoch in range(N_EPOCHS):
        # ── Unfreeze last DINOv2 block at the configured epoch ──────────────
        if not backbone_unfrozen and (epoch + 1) >= BACKBONE_UNFREEZE_EPOCH:
            ...
            print(f"\n  * Epoch {epoch+1}: Unfroze last DINOv2 block "
                f"({n_new/1e6:.2f}M params, lr={BACKBONE_LR:.2e})")

        head.train()
        # Keep backbone in train mode for last block if unfrozen, eval otherwise
        ...
        for step, (imgs, labels) in enumerate(pbar):
            ...
            with autocast():
                # Use no_grad only while backbone is fully frozen; once the last
                # block is unfrozen we need gradients to flow through it.
                if backbone_unfrozen:
                    feats = backbone.forward_features(imgs)["x_norm_patchtokens"]
                else:
                    with torch.no_grad():
            ...
            if (step + 1) % ACCUM_STEPS == 0 or (step + 1) == len(train_loader):
                ...
                # Clip grads across head AND any unfrozen backbone params
        ...
        if val_miou > best_iou:
            ...
            torch.save({
                ...
                'backbone_state_dict': backbone.blocks[-1].state_dict() if backbone_unfrozen else None,
